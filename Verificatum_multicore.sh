#!/bin/bash
# Multi-core Verificatum benchmark.
# Uses default JVM + Verificatum parallelism.
...
echo "Verificatum benchmark (multi-threaded, $NUM_CORES threads)"
...
vmni -prot -sid "BenchMC" -name "BenchmarkMultiCore" \
...
for N in "${N_VALUES[@]}"; do
    ...
    echo " cores=$NUM_CORES  |  mode=$MODE"
    ...
    {
        echo ""
        echo "--- Summary N=$N  Mode=$MODE  Cores=$NUM_CORES  (Prover: $PC runs, Verifier: $VC runs) ---"
